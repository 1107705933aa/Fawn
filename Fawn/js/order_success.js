...
window.onload = function() {
	...
	let if_fu = document.querySelectorAll('.if_fu');
	let payy = document.querySelectorAll('.payy');
	console.log(if_fu)
	for(let s = 0; s < if_fu.length; s++) {
		if(if_fu[s].innerText == '已付') {
			//		    payy.dataset.x = '';		
			console.log(if_fu[s])
			payy[s].setAttribute("href", "#");
		}
	}
	delOrderr();
	dingdan();
...
function dingdan() {
	...
	let dingzhuo = document.querySelectorAll('.dingzhuo');
	let youge = document.querySelectorAll('.youge')
	for(let j = 0; j < dingzhuo.length; j++) {
		console.log(dingdan[0].innerHTML);
		if(dingdan[j].innerHTML == null || dingdan[j].innerHTML == "" || dingdan[j].innerHTML == undefined) {
			dingzhuo[j].style.display = "none";
			//			youge[j].style.height = '20px';
			//			youge[j].style.width = '100%';
		}
	}
}
